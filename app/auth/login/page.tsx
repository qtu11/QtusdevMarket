...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save user info for next login - comprehensive storage
      localStorage.setItem("currentUser", JSON.stringify(data.user));
      localStorage.setItem("isLoggedIn", "true");
      localStorage.setItem("qtusdev_user", JSON.stringify(data.user));
      localStorage.setItem("lastLoginTime", new Date().toISOString());
      
      // Save to registered users for future logins
      const registeredUsers = JSON.parse(localStorage.getItem("registeredUsers") || "[]");
      const existingUserIndex = registeredUsers.findIndex((u: any) => u.email === data.user.email);
      if (existingUserIndex >= 0) {
        registeredUsers[existingUserIndex] = data.user;
      } else {
