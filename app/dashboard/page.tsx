...
import { getDeviceInfo, getIPAddress } from "@/lib/telegram"
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        setUserIP('Unknown')
        setDeviceInfo({ deviceType: 'Unknown', browser: 'Unknown', os: 'Unknown' })
      }
    }
  ...
  useEffect(() => {
    // Comprehensive user authentication check with fallback support
    ...
    console.log('🔍 Dashboard auth check:', {
      isLoggedIn,
      hasUser: !!userStr,
      userSource: userStr ? 'Found' : 'Missing'
    })
    ...
    try {
      ...
      // Comprehensive user data sync from multiple sources
      const registeredUsers = JSON.parse(localStorage.getItem('registeredUsers') || '[]')
      const socialUsers = JSON.parse(localStorage.getItem('socialUsers') || '[]')
      const allUsers = JSON.parse(localStorage.getItem('users') || '[]')
      
      // Find the most recent user data from all sources
      const latestUser = registeredUsers.find((u: any) => u.email === user.email) ||
                        socialUsers.find((u: any) => u.email === user.email) ||
      ...
      completeUser = {
        ...latestUser,
        uid: latestUser.uid || user.uid,
        email: latestUser.email || user.email,
        name: latestUser.name || user.name,
        provider: latestUser.provider || user.provider || 'email',
        balance: latestUser.balance || user.balance || 0,
        loginCount: latestUser.loginCount || user.loginCount || 1,
        lastLogin: latestUser.lastLogin || user.lastLogin || new Date().toISOString(),
        ipAddress: userIP !== "Loading..." ? userIP : (latestUser.ipAddress || user.ipAddress || 'Unknown'),
        deviceInfo: deviceInfo || user.deviceInfo || latestUser.deviceInfo,
        status: latestUser.status || user.status || 'active'
      }
      
      setCurrentUser(completeUser)
      // Update currentUser with latest data
      localStorage.setItem("currentUser", JSON.stringify(completeUser))
      localStorage.setItem("qtusdev_user", JSON.stringify(completeUser))
    ...
    // Load user's purchases with comprehensive user ID matching
    const allPurchases = JSON.parse(localStorage.getItem("userPurchases") || "[]")
    const userPurchasesList = allPurchases.filter((purchase: any) => 
      purchase.userId === completeUser.uid || 
      purchase.userId === completeUser.id || 
      purchase.userEmail === completeUser.email
    )
    setUserPurchases(userPurchasesList)
    console.log('📦 User purchases loaded:', userPurchasesList.length)

    // Load deposit history with comprehensive user ID matching
    const allDeposits = JSON.parse(localStorage.getItem("approvedDeposits") || "[]")
    const pendingDeposits = JSON.parse(localStorage.getItem("deposits") || "[]")
    const allDepositsCombined = [...allDeposits, ...pendingDeposits]
    
    const userDeposits = allDepositsCombined.filter((deposit: any) => 
      deposit.userId === completeUser.uid || 
      deposit.userId === completeUser.id || 
      deposit.userEmail === completeUser.email ||
      deposit.userEmail === completeUser.email
    )
    setDepositHistory(userDeposits)
    console.log('💰 User deposits loaded:', userDeposits.length, 'from', allDepositsCombined.length, 'total')

    // Load withdrawal history with comprehensive user ID matching
    const allWithdrawals = JSON.parse(localStorage.getItem("approvedWithdrawals") || "[]")
    const pendingWithdrawals = JSON.parse(localStorage.getItem("withdrawals") || "[]")
    ...
    const userWithdrawals = allWithdrawalsCombined.filter((withdrawal: any) => 
      withdrawal.userId === completeUser.uid || 
      withdrawal.userId === completeUser.id || 
      withdrawal.userEmail === completeUser.email ||
      withdrawal.userEmail === completeUser.email
    )
    setWithdrawHistory(userWithdrawals)
    console.log('💸 User withdrawals loaded:', userWithdrawals.length, 'from', allWithdrawalsCombined.length, 'total')

    // Send notification when dashboard is accessed
    const now = new Date().toLocaleString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" });
    const message = `📱 Người dùng ${completeUser.email} đã truy cập dashboard vào lúc ${now} từ IP: ${completeUser.ipAddress || "Unknown"}`;
    sendNotification(message);

    setIsLoading(false)

    // Set up real-time updates with comprehensive data sync
    const interval = setInterval(() => {
      try {
        // Refresh user data from all sources
        const registeredUsers = JSON.parse(localStorage.getItem('registeredUsers') || '[]')
        const socialUsers = JSON.parse(localStorage.getItem('socialUsers') || '[]')
        const allUsers = JSON.parse(localStorage.getItem('users') || '[]')
        const updatedUserStr = localStorage.getItem("currentUser") || localStorage.getItem("qtusdev_user")
        
        if (updatedUserStr) {
          const currentUser = JSON.parse(updatedUserStr)
          
          // Find the most recent user data from all sources
          const latestUser = registeredUsers.find((u: any) => u.email === currentUser.email) ||
                            socialUsers.find((u: any) => u.email === currentUser.email) ||
                            allUsers.find((u: any) => u.email === currentUser.email) ||
                            currentUser
          
          // Ensure all required fields are present
          const completeUser = {
            ...latestUser,
            uid: latestUser.uid || currentUser.uid,
            email: latestUser.email || currentUser.email,
            name: latestUser.name || currentUser.name,
            provider: latestUser.provider || currentUser.provider || 'email',
            balance: latestUser.balance || currentUser.balance || 0
          }
        ...
        // Refresh purchases with comprehensive user ID matching
        const updatedPurchases = JSON.parse(localStorage.getItem("userPurchases") || "[]")
        const userPurchasesList = updatedPurchases.filter((purchase: any) => 
          purchase.userId === currentUser.uid || 
          purchase.userId === currentUser.id || 
          purchase.userEmail === currentUser.email
        )
        setUserPurchases(userPurchasesList)

        // Refresh deposits with comprehensive user ID matching
        const updatedDeposits = JSON.parse(localStorage.getItem("approvedDeposits") || "[]")
        const updatedPendingDeposits = JSON.parse(localStorage.getItem("deposits") || "[]")
        const allUpdatedDeposits = [...updatedDeposits, ...updatedPendingDeposits]
        
        const userDeposits = allUpdatedDeposits.filter((deposit: any) => 
          deposit.userId === currentUser.uid || 
          deposit.userId === currentUser.id || 
          deposit.userEmail === currentUser.email
        )
        setDepositHistory(userDeposits)

        // Refresh withdrawals with comprehensive user ID matching
        const updatedWithdrawals = JSON.parse(localStorage.getItem("approvedWithdrawals") || "[]")
        const updatedPendingWithdrawals = JSON.parse(localStorage.getItem("withdrawals") || "[]")
        const allUpdatedWithdrawals = [...updatedWithdrawals, ...updatedPendingWithdrawals]
        
        const userWithdrawals = allUpdatedWithdrawals.filter((withdrawal: any) => 
          withdrawal.userId === currentUser.uid || 
          withdrawal.userId === currentUser.id || 
          withdrawal.userEmail === currentUser.email
        )
        setWithdrawHistory(userWithdrawals)
      } catch (error) {
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {new Date(currentUser.createdAt || currentUser.joinDate || new Date()).toLocaleDateString('vi-VN')}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {currentUser.lastLogin || currentUser.lastLoginTime || currentUser.lastActivity || "Chưa có thông tin"}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {userIP !== "Loading..." ? userIP : (currentUser.ipAddress || currentUser.currentIP || "Unknown")}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {deviceInfo?.deviceType || currentUser.currentDeviceInfo?.deviceType || currentUser.deviceInfo?.deviceType || "Unknown"}
                        </p>
                      </div>
                    </div>
                    {deviceInfo && (
                      <div className="flex items-center space-x-3">
                        <Settings className="w-4 h-4 text-muted-foreground" />
                        <div>
                          <p className="text-sm text-muted-foreground">Trình duyệt</p>
