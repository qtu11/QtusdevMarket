...
import { sendDepositNotification, getDeviceInfo, getIPAddress } from "@/lib/telegram"
...
export default function DepositPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send Telegram notification with comprehensive info
      const telegramMessage = `💳 <b>YÊU CẦU NẠP TIỀN MỚI</b>

👤 <b>Khách hàng:</b> ${user.name}
📧 <b>Email:</b> ${user.email}  
💰 <b>Số tiền:</b> ${depositAmount.toLocaleString("vi-VN")}đ
🏦 <b>Ngân hàng:</b> ${method.name}
📝 <b>Mã giao dịch:</b> ${transactionId}
🏦 <b>Tên chủ TK:</b> ${method.accountName}
🔢 <b>Số TK:</b> ${method.accountNumber}
🌐 <b>IP:</b> ${ipAddress}
...
<i>Vui lòng kiểm tra và duyệt yêu cầu!</i>`
      ...
      try {
        const telegramSent = await sendDepositNotification({
          userName: user.name,
          userEmail: user.email,
          amount: depositAmount,
          method: method.name,
