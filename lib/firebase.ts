...
// Initialize Firebase (with fallback if not configured)
let app: any;
...
try {
  ...
  if (firebaseConfig.apiKey && firebaseConfig.authDomain && firebaseConfig.projectId) {
    ...
    console.warn("Firebase configuration is incomplete. Using localStorage fallback.");
  }
} catch (error) {
...
export const signUpWithEmail = async (email: string, password: string, userData: { name: string }): Promise<{ user: User | null; error: string | null }> => {
  ...
  try {
    ...
    return { user: userWithoutPassword as any, error: null };
  } catch (error) {
